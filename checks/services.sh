...
check_services() {
  local failed=0 names="" var_root="${RD_VAR_ROOT:-/var}"
  ...
  if [[ -e "$var_root/run/reboot-required" ]]; then
    rd_add_result "SVC-002" "Services" "warn" "low" "Pending reboot" "The operating system reports that a reboot is required." "$var_root/run/reboot-required exists" "Schedule a controlled reboot after checking active workloads." ""
  else
